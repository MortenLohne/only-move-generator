...
use shakmaty::{Board, Chess, Color, FromSetup, Piece, Position, Role, fen};
...
fn main() {
    ...
    iter::repeat_with(|| generate_6man_with_eval(&tables, &mut rng))
        ...
        .filter_map(|(pos, wdl, dtz)| {
            ...
            children.sort_by_key(|(_, child_dtz)| child_dtz.clone());
            children.reverse();
        ...
        .for_each(|(pos, wdl, dtz, mv)| {
            match wdl  {
                Wdl::Win => println!(
                        "{} hmvc {}; bm {}",
                        fen::epd(&pos), 100 - dtz.0, mv
                    ),
                Wdl::Loss => println!(
                    "{} hmvc {}; bm {}",
                    fen::epd(&pos), 100 - dtz.0.abs() + 1, mv
                ),
                _ => unreachable!(),
            }
        });
...
fn generate_6man_with_eval<R: Rng>(tables: &Tablebase<Chess>, rng: &mut R) -> (Chess, Wdl, Dtz) {
    let pos: Chess = generate_random_6man(rng).into();
...
fn generate_random_6man<R: Rng>(rng: &mut R) -> Chess {
    ...
    for _ in 0..4 {
        ...
        let color = match rng.gen_bool(0.5) {
            true => Color::White,
            false => Color::Black,
        };
        loop {
